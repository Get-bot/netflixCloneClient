  ...
  const accordionItems: accordionContent[] = [
    {
      ...
      content: `넷플릭스는 장편 영화, 다큐멘터리, 시리즈, 애니메이션, 각종 상을 수상한 넷플릭스 오리지널 등 수많은 콘텐츠를 확보하고 있습니다. 마음에 드는 콘텐츠를 원하는 시간에 원하는 만큼 시청하실 수 있습니다.<br/><br/>넷플릭스 콘텐츠를 한번 살펴보세요.`,
    },
    {
      title: "넷플릭스란 무엇인가요?",
...
return (
  <>
    <div className={classes.cardFaqContainer}>
      <div className={classes.cardFaqContentContainer}>
        <div className={classes.cardFaqContent}>
          <h2 className={classes.cardFaqTextTitle}>자주 묻는 질문</h2>
          <div className={classes.cardFaqAccordionContainer}>
            <ul className={classes.cardFaqAccordion}>
              {accordionItems.map((item, index) => {
                return <FaqAccordionItem key={index} index={index} title={item.title} content={item.content}/>
              })
              }
            </ul>
            <CreateAccount className={"createAccountFaqContainer"} labelId={"faq-email"} id={"faq-email"}/>
          </div>
          <div className="default-ltr-cache-vgp0nn e9eyrqp3"></div>
        </div>
        <div data-uia="nmhp-card-faq+divider" className="default-ltr-cache-1yz1b0z e9eyrqp0"></div>
      </div>
      <SeparationLine/>
    </div>
  </>
